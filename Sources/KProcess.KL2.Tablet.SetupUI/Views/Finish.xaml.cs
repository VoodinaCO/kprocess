...
namespace KProcess.KL2.Tablet.SetupUI.Views
{
    ...
    public partial class Finish : InstallScreen
    {
        ...
        public ICommand ExitButtonCommand { get; } = new DelegateCommand<object>(e =>
        {
            try
            {
                string exePath = string.IsNullOrEmpty(MainViewModel.Instance.KL2InstallPath) ? MainViewModel.Instance.Default_KL2InstallPath : MainViewModel.Instance.FormattedKL2InstallPath;
                if (MainViewModel.Instance.LaunchKL2OnExit && ManagedBA.Instance.ActionResult == ActionResult.Success)
                {
                    Process startProcess = new Process();
                    startProcess.StartInfo.FileName = Path.Combine(exePath, "Kprocess.KL2.TabletClient.exe");
