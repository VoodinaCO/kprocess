...
namespace Kprocess.KL2.FileTransfer
{
    ...
    {
        readonly ConcurrentDictionary<TKey, TValue> dictionary;
        ...
        public void AddWithNotification(KeyValuePair<TKey, TValue> item) =>
            AddWithNotification(item.Key, item.Value);

        public void AddWithNotification(params KeyValuePair<TKey, TValue>[] items)
        {
            foreach (var item in items)
                AddWithNotification(item.Key, item.Value);
        }

        public void AddWithNotification(IEnumerable<KeyValuePair<TKey, TValue>> items)
        {
            foreach (var item in items)
                AddWithNotification(item.Key, item.Value);
        }

        public void AddWithNotification(TKey key, TValue value)
        {
        ...
        public bool RemoveWithNotification(TKey key)
        {
        ...
        public bool RemoveWithNotification(params TKey[] keys)
        {
            bool result = false;
            foreach (var key in keys)
                result |= RemoveWithNotification(key);
            return result;
        }

        public bool RemoveWithNotification(IEnumerable<TKey> keys)
        {
            bool result = false;
            foreach (var key in keys)
                result |= RemoveWithNotification(key);
        ...
        public bool RemoveWithNotification(Func<KeyValuePair<TKey,TValue>,bool> predicate)
        {
            var keys = dictionary.ToList().Where(predicate).Select(_ => _.Key);
            return RemoveWithNotification(keys);
        }
        ...
        int ICollection<KeyValuePair<TKey, TValue>>.Count =>
            ((ICollection<KeyValuePair<TKey, TValue>>)dictionary).Count;
