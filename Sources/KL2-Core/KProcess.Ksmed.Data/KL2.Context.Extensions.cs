...
namespace KProcess.Ksmed.Data
{
    public static class SelfTrackingEntitiesContextExtensions
    {
        ...
        private static object GetValue(this System.Data.Entity.Core.Objects.DataClasses.EntityReference entityReference)
        {
        ...
        private static EntityKey GetCurrentEntityKey(this System.Data.Entity.Core.Objects.DataClasses.EntityReference entityReference, ObjectContext context)
        {
        ...
        private sealed class AddHelper
        {
            ...
            private readonly Queue<(string EntitySetName, IObjectWithChangeTracker Entity)> _entitiesToAdd;
            private readonly Queue<(ObjectStateEntry Entry, string PropertyName, IEnumerable<object> PropertyValue)> _entitiesDuringAdd;
            ...
            public static EntityIndex AddAllEntities(ObjectContext context, string entitySetName, IObjectWithChangeTracker entity)
            {
                ...
                try
                {
                    ...
                    {
                        var entityInSet = addHelper.NextAdd();
                        // Only add the object if it's not already in the context
                        ObjectStateEntry entry = null;
                        if (entityInSet.Entity == null)
                            continue;
                        if (!context.ObjectStateManager.TryGetObjectStateEntry(entityInSet.Entity, out entry))
                            context.AddObject(entityInSet.EntitySetName, entityInSet.Entity);
                    }
                }
                finally
                {
            ...
            private AddHelper(ObjectContext context)
            {
                ...
                _entitiesToAdd = new Queue<(string EntitySetName, IObjectWithChangeTracker Entity)>();
                _entitiesDuringAdd = new Queue<(ObjectStateEntry Entry, string PropertyName, IEnumerable<object> PropertyValue)>();
            }
            ...
            private void QueueAdd(string entitySetName, IObjectWithChangeTracker entity)
            {
                if (!_entityIndex.Contains(entity))
                {
                    ...
                    _entitiesToAdd.Enqueue((entitySetName, entity));
                }
            }
    
            private (string EntitySetName, IObjectWithChangeTracker Entity) NextAdd()
            {
                ProcessNewAdds();
            ...
            private void ProcessNewAdds()
            {
                ...
                {
                    (ObjectStateEntry Entry, string PropertyName, IEnumerable<object> PropertyValue) relatedEntities = _entitiesDuringAdd.Dequeue();
                    RelatedEnd relatedEnd = relatedEntities.Entry.GetRelatedEnd(relatedEntities.PropertyName);
                    string entitySetName = relatedEnd.GetEntitySetName();
    
                    foreach (var targetEntity in relatedEntities.PropertyValue)
                    {
