...
namespace AForge.Video.DirectShow
{
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        VideoCapabilities[] snapshotCapabilities;

        bool needToSetVideoInput;
        bool needToSimulateTrigger;
        bool needToDisplayPropertyPage;
        bool needToDisplayCrossBarPropertyPage;
        IntPtr parentWindowForPropertyPage = IntPtr.Zero;
        ...
        VideoInput[] crossbarVideoInputs;
        VideoInput crossbarVideoInput = VideoInput.Default;

        // cache for video/snapshot capabilities and video inputs
        static Dictionary<string, VideoCapabilities[]> cacheVideoCapabilities = new Dictionary<string, VideoCapabilities[]>();
        static Dictionary<string, VideoCapabilities[]> cacheSnapshotCapabilities = new Dictionary<string, VideoCapabilities[]>();
        static Dictionary<string, VideoInput[]> cacheCrossbarVideoInputs = new Dictionary<string, VideoInput[]>();
        ...
        /// </remarks>
        ///
        public bool ProvideSnapshots { get; set; } = false;
        ...
        /// </remarks>
        /// 
        public object SourceObject { get; private set; } = null;
        ...
        private void WorkerThread(bool runGraph)
        {
            ...
            try
            {
                ...
                GCHandle mediaTypeHandle = GCHandle.Alloc(mediaType);
                ...
                if (videoControl != null)
                {
                    ...
                    captureGraph.FindPin(SourceObject, PinDirection.Output,
                        PinCategory.Still, MediaType.Video, false, 0, out pinStillImage);
                ...
                {
                    GetPinCapabilitiesAndConfigureSizeAndRate(captureGraph, sourceBase,
                        PinCategory.Still, SnapshotResolution, ref snapshotCapabilities);
                }
                else
                {
                ...
                if (runGraph)
                {
                    ...
                    {
                        // render snapshot pin
                        captureGraph.RenderStream(PinCategory.Still, MediaType.Video, sourceBase, null, snapshotGrabberBase);
                    ...
                    do
                    {
                        if (mediaEvent != null)
                        {
                            if (mediaEvent.GetEvent(out EventCode code, out IntPtr p1, out IntPtr p2, 0) >= 0)
                            {
        ...
        private void SetResolution(IAMStreamConfig streamConfig, VideoCapabilities resolution)
        {
            ...
            GCHandle newMediaTypeHandle = GCHandle.Alloc(newMediaType);
            VideoStreamConfigCaps caps = new VideoStreamConfigCaps();
            GCHandle capsHandle = GCHandle.Alloc(caps);
            ...
            for (int i = 0; i < capabilitiesCount; i++)
            {
                try
                {
                    ...
                    if (resolution == vc)
                    {
                        if (streamConfig.GetStreamCaps(i, out newMediaType, (IntPtr)capsHandle) == 0)
                        {
                            break;
                        }
                    }
        ...
        void GetPinCapabilitiesAndConfigureSizeAndRate(ICaptureGraphBuilder2 graphBuilder, IBaseFilter baseFilter,
        ...
        void DisplayPropertyPage(IntPtr parentWindow, object sourceObject)
        {
            try
            {
                ...
                pPropPages.GetPages(out DsCAUUID caGUID);
        ...
        VideoInput[] ColletCrossbarVideoInputs(IAMCrossbar crossbar)
        {
            lock (cacheCrossbarVideoInputs)
            {
        ...
        VideoInput GetCurrentCrossbarInput(IAMCrossbar crossbar)
        {
        ...
        void SetCurrentCrossbarInput(IAMCrossbar crossbar, VideoInput videoInput)
        {
            if (videoInput.Type != 0)
            {
                // gen number of pins in the crossbar
                if (crossbar.get_PinCounts(out int outPinsCount, out int inPinsCount) == 0)
                {
        ...
        /// 
        void OnSnapshotFrame(Bitmap image)
        {
        ...
        {
            readonly VideoCaptureDevice _parent;
            readonly bool _snapshotMode;
