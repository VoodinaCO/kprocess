    ...
    public class TreeMap : Control
    {
        ...
        protected virtual void OnItemsSourcePropertyChanged(IEnumerable oldValue, IEnumerable newValue)
        {
            ...
            if (newValue is INotifyCollectionChanged newValueINotifyCollectionChanged)
            {
                ...
                {
                    OnEventAction = (instance, source, eventArgs) => instance.ItemsSourceCollectionChanged(source, eventArgs),
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (_nodeRoots != null && ContainerElement != null)
            {
                ...
                IEnumerable<(Rect RectangularArea, TreeMapNode TreeMapNode)> measuredRectangles = ComputeRectangles(
                    root,
                    new Rect(0, 0, ContainerElement.ActualWidth, ContainerElement.ActualHeight));

                // Position everything
                foreach (var rectangle in measuredRectangles)
                {
                    FrameworkElement element = rectangle.TreeMapNode.Element;
                    if (element != null)
                    {
                        double roundedTop = Math.Round(rectangle.RectangularArea.Top);
                        double roundedLeft = Math.Round(rectangle.RectangularArea.Left);
                        double height = Math.Round(rectangle.RectangularArea.Height + rectangle.RectangularArea.Top) - roundedTop;
                        double width = Math.Round(rectangle.RectangularArea.Width + rectangle.RectangularArea.Left) - roundedLeft;
        ...
        private IEnumerable<TreeMapNode> BuildTreeMapTree(IEnumerable nodes, int level)
        {
            ...
            foreach (object root in nodes)
            {
                ...
                if (template.ValueBinding != null)
                {
                    ...
                    if (objectChildren is INotifyCollectionChanged objectChildrenINotifyCollectionChanged)
                    {
                        // Use a WeakEventListener so that the backwards reference doesn't keep this object alive
                        weakEventListener = new WeakEventListener<TreeMap, object, NotifyCollectionChangedEventArgs>(this)
                        {
                            OnEventAction = (instance, source, eventArgs) => instance.ItemsSourceCollectionChanged(source, eventArgs),
                            OnDetachAction = (wel) => objectChildrenINotifyCollectionChanged.CollectionChanged -= wel.OnEvent
                        };
            ...
            foreach (TreeMapNode current in GetTreeMapNodes())
            {
                ...
                if (element != null)
                {
                    ...
                    foreach (Interpolator interpolator in Interpolators)
                    {
                        ...
                        {
                            if (element.FindName(interpolator.TargetName) is DependencyObject target)
                            {
                                SetBinding(
