...
namespace System.Windows.Controls.DataVisualization
{
    ...
    internal class SquaringAlgorithm
    {
        ...
        private IEnumerable<(Rect RectangularArea, TreeMapNode TreeMapNode)> BuildTreeMap()
        {
            _currentStart = 0;
            while (_currentStart < _areas.Count)
            {
                foreach (var rectangle in BuildTreeMapStep())
                {
        ...
        private IEnumerable<(Rect RectangularArea, TreeMapNode TreeMapNode)> BuildTreeMapStep()
        {
