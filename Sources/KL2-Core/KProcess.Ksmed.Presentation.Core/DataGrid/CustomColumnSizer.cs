...
namespace KProcess.Ksmed.Presentation.Core
{
    ...
    public class CustomColumnSizer : GridColumnSizer
    {
        ...
        protected override FormattedText GetFormattedText(GridColumn column, object record, string datatext) =>
            GetFormattedTextInternal(datatext, column);

        FormattedText GetFormattedTextInternal(string datatext, GridColumn column)
        {
            var fontFamily = DataGrid.FontFamily;
            var fontStyle = DataGrid.FontStyle;
            var fontWeight = DataGrid.FontWeight;
            var fontStretch = DataGrid.FontStretch;
            var fontSize = DataGrid.FontSize;
            FormattedText formattedtext;
            formattedtext = new FormattedText(datatext, System.Globalization.CultureInfo.CurrentCulture, DataGrid.FlowDirection, new Typeface(fontFamily, fontStyle, fontWeight, fontStretch), fontSize, Brushes.Black);
        ...
        public Size MeasureText(string datatext, GridColumn column, bool ignoreWidth = false)
        {
            ...
            FormattedText formattedtext = GetFormattedTextInternal(datatext, column);
