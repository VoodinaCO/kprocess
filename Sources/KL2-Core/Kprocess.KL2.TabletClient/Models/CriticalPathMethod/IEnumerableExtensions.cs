...
namespace Kprocess.KL2.TabletClient.Models.CriticalPathMethod
{
    public static class IEnumerableExtensions
    {
        ...
        static IEnumerable<KeyValuePair<T, PathInfo<T>>> OrderByDependencies<T>(IEnumerable<KeyValuePair<T, PathInfo<T>>> list)
        {
        ...
        static void FillEarliestValues<T>(this IDictionary<T, PathInfo<T>> list, Func<T, long> lengthSelector)
        {
            if (!list.Any()) return;
        ...
        /// <param name="lengthSelector">Function to determine the length, duration, etc for the node.  
        /// In the absence of INumeric, since we have to do math, we're going to force a long return value</param>
        static void FillLatestValues<T>(this IDictionary<T, PathInfo<T>> list, Func<T, long> lengthSelector)
        {
        ...
        static IDictionary<T, IEnumerable<T>> GetSucessors<T>(this IEnumerable<T> list, Func<T, IEnumerable<T>> predecessorSelector)
        {
        ...
        static IDictionary<T, PathInfo<T>> ToPathInfoDictionary<T>(this IEnumerable<T> list, Func<T, IEnumerable<T>> predecessorSelector, Func<T, IEnumerable<T>> sucessorSelector)
        {
