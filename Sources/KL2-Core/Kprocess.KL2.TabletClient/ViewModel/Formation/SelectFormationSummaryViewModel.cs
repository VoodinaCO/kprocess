...
namespace Kprocess.KL2.TabletClient.ViewModel
{
    ...
    /// </para>
    /// </summary>
    public class SelectFormationSummaryViewModel : ViewModelBase, IRefreshViewModel, IMediaElementViewModel, ISfDataGridViewModel<PublishedAction>
    {
        ...
        BindingList<User> _operators = new BindingList<User>();
        public BindingList<User> Operators
        {
            get => _operators;
            private set
            {
        ...
        BindingList<User> _trainedOperators = new BindingList<User>();
        public BindingList<User> TrainedOperators
        {
            get => _trainedOperators;
            private set
            {
                if (_trainedOperators != value)
                {
                    _trainedOperators = value;
                    RaisePropertyChanged();
                }
            }
        }
        ...
        public Publication Publication
        {
            ...
            set
            {
                ...
                if (Locator.Main.TrainingPublication != null)
        ...
        public bool PublicationIsRead =>
            Publication.Readers.Any(_ => _.UserId == Locator.Main.SelectedUser.UserId && _.ReadDate != null);
        ...
        async void OnCriticalPathCurrentPublishedActionChanged()
        {
            if (_criticalPathCurrentPublishedAction != null)
            {
                ...
                {
                    await Locator.Navigation.PushDialog<CriticalPathDialog, SelectFormationSummaryViewModel>(this);
        ...
        public ICommand CloseCriticalPathDialogCommand
        {
            get
            {
                if (_closeCriticalPathDialogCommand == null)
                    ...
                    {
                        if (MediaElement != null && MediaElement.MediaState != Unosquare.FFME.Common.MediaPlaybackState.Close)
                ...
                if (_showStepCommand == null)
                    _showStepCommand = new RelayCommand<PublishedAction>(async (publishedAction) =>
                    {
                        ...
                        try
                        {
                            ...
                            await Locator.Navigation.PushDialog<FormationSummaryDetailsDialog, SelectFormationSummaryViewModel>(this);
                        }
                        catch (Exception ex)
                        {
        ...
        ICommand _setReadCommand;
        public ICommand SetReadCommand
        {
            get
            {
                if (_setReadCommand == null)
                    _setReadCommand = new RelayCommand(async () =>
                    {
                        readCts = new CancellationTokenSource();
                        try
                        {
                            await Task.Delay(TimeSpan.FromSeconds(2), readCts.Token);

                            Publication.SetRead(Locator.Main.SelectedUser.UserId);
                            RaisePropertyChanged(nameof(PublicationIsRead));
                        }
                        catch (TaskCanceledException) { }
                    });

                return _setReadCommand;
            }
        }

        ICommand _setUnreadCommand;
        public ICommand SetUnreadCommand
        {
            get
            {
                if (_setUnreadCommand == null)
                    _setUnreadCommand = new RelayCommand(() =>
                    {
                        readCts?.Cancel();
                    });

                return _setUnreadCommand;
            }
        }
