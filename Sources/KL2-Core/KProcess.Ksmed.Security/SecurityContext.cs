...
namespace KProcess.Ksmed.Security
{
    ...
    public static class SecurityContext
    {
        ...
        /// </summary>
        public static SecurityUser CurrentUser { get; internal set; }
        ...
        /// <param name="language">La langue.</param>
        public static async Task<(User User, bool Result)> TryLogonUser(string username, string password, string language = null)
        {
            if (!_isInitialized)
                throw new Exception("Contexte non initialisé");

            // Récupérer l'utilisateur en base
            TraceManager.TraceDebug($"SecurityContext.TryLogonUser : tentative de connexion : {username}");
            ...
            try
            {
                ...
                if (user != null)
                {
                    TraceManager.TraceDebug($"SecurityContext.TryLogonUser : utilisateur trouvé : {user.Username}");

                    // Authentifie l'utilisateur
                    var apiHttpClient = IoC.Resolve<IAPIHttpClient>();
                    if (await apiHttpClient.Logon(username, password ?? string.Empty, language))
                    {
                        TraceManager.TraceDebug("SecurityContext.TryLogonUser : connexion réussie");
                        CurrentUser = new SecurityUser(user);
                        // Meanwhile complete migration, two different context are used to store current user
                        IoC.Resolve<ISecurityContext>().CurrentUser = CurrentUser;
                        return (user, true);
                    }
                    return (null, false);
                }
        ...
        /// </summary>
        public static async Task AutoLogAsDefaultAdmin()
        {
        ...
        public static void AutoLogAsForTests(User user)
        {
            CurrentUser = new SecurityUser(user);
            // Meanwhile complete migration, two different context are used to store current user
            IoC.Resolve<ISecurityContext>().CurrentUser = CurrentUser;
        ...
        public static void DisconnectCurrentUser()
        {
            ...
            IoC.Resolve<IAPIHttpClient>().Token = null;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static IAuthenticationMode GetAuthenticationMode()
        {
        ...
        public static bool CurrentAuthenticationModeNeedsDomain() =>
            GetAuthenticationMode().NeedsDomain;

        /// <summary>
        /// Obtient ou définit les informations de la licence actuellement chargée.
        /// </summary>
        public static Activation.WebProductLicense CurrentProductLicense { get; set; }
        ...
        /// </returns>
        public static bool CanUserRead<T>()
        {
            ...
            if (canRoleRead)
            {
                ...
                if (CurrentProductLicense == null)
                    ...
                    canRoleRead |= _featuresReadAuthorizations[typeof(T)].Contains((short)ActivationFeatures.All);
            }
        ...
        /// </returns>
        public static bool CanRoleRead<T>(string role)
        {
            if (_rolesReadAuthorizations == null)
                throw new InvalidOperationException("Les autorisations n'ont pas encore été enregistrées.");
        ...
        /// </returns>
        public static bool CanUserWrite<T>()
        {
            ...
            if (canRoleWrite)
            {
                ...
                if (CurrentProductLicense == null)
                    ...
                    canRoleWrite |= _featuresWriteAuthorizations[typeof(T)].Contains((short)ActivationFeatures.All);
            }
        ...
        /// </returns>
        public static bool CanRoleWrite<T>(string role)
        {
            if (_rolesWriteAuthorizations == null)
                throw new InvalidOperationException("Les autorisations n'ont pas encore été enregistrées.");
        ...
        /// </returns>
        public static bool HasCurrentUserRole(string role) =>
        ...
        /// </returns>
        public static bool HasCurrentLicenseFeature(Activation.ActivationFeatures feature)
        {
            if (CurrentProductLicense == null)
                return false;
            else
            {
                if (CurrentProductLicense.Status == WebLicenseStatus.Expired)
                {

                    if (feature == ActivationFeatures.ReadOnly)
                        return true;
                    else
                        return false;
                }
        ...
        /// </summary>
        public static void RegisterAuthorizations(
        ...
        public static bool IsUserDefaultAdmin(User user)
        {
