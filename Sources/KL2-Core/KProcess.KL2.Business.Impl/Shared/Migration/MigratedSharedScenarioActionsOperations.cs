...
namespace KProcess.KL2.Business.Impl.Shared.Migration
{
    ...
    /// </summary>
    public class MigratedSharedScenarioActionsOperations : ISharedScenarioActionsOperations
    {
        private readonly ITraceManager _traceManager;
        private readonly ISecurityContext _securityContext;
        private readonly ILocalizationManager _localizationManager;
        public MigratedSharedScenarioActionsOperations(
        ...
        /// Les données
        /// </returns>
        public async Task<AcquireData> GetAcquireData(KsmedEntities context, int projectId, GetDataScenarioNatures natureFilter)
        {
            ...
            AcquireData data = new AcquireData
            {
                ...
                Videos = await context.Projects
                    ...
                    .Where(v => v.OnServer == true && v.Sync == true)
        ...
        /// <param name="allScenarios">Tous les scénarios liés.</param>
        /// <param name="recursive"><c>true</c> pour appliquer les changements récursivement sur les scénarios dérivés.</param>
        public Task SaveBuildScenario(KsmedEntities context, Scenario[] allScenarios, Scenario updatedScenario, bool recursive)
        {
        ...
        /// Les données
        /// </returns>
        public async Task<SimulateData> GetSimulateData(KsmedEntities context, int projectId, GetDataScenarioNatures natureFilter)
        {
            IDictionary<ProcessReferentialIdentifier, bool> referentialsUsed = await GetReferentialsUse(context, projectId);
        ...
        private async Task<Scenario[]> LoadScenarios(KsmedEntities context, int projectId, IDictionary<ProcessReferentialIdentifier, bool> referentialsUsed, GetDataScenarioNatures natureFilter)
        {
            Scenario[] scenarios;
            ...
            switch (natureFilter)
            {
                case GetDataScenarioNatures.InitialAndTarget:
                    scenarios = await query
                        .Where(s => s.ProjectId == projectId && (s.NatureCode == KnownScenarioNatures.Initial || s.NatureCode == KnownScenarioNatures.Target) && !s.IsDeleted)
                        .ToArrayAsync();
                    break;

                case GetDataScenarioNatures.Realized:
                    scenarios = await query
                        .Where(s => s.ProjectId == projectId && s.NatureCode == KnownScenarioNatures.Realized && !s.IsDeleted)
                        .ToArrayAsync();
                    break;
        ...
        public async Task<Ksmed.Business.Migration.ActionDuration[]> GetActionsBuildDurations(KsmedEntities context, IEnumerable<int> ids)
        {
            ...
            return (await new ObjectQuery<DbDataRecord>(query.ToString(), context).ExecuteAsync(MergeOption.NoTracking))
                .Select(dbdr => new Ksmed.Business.Migration.ActionDuration
                {
        ...
        public void ApplyReduced(KAction originalAction, KAction newAction)
        {
        ...
        public void ApplyNewReduced(KAction action, string actionTypeCode = null)
        {
        ...
        public void EnsureEmptySolutionExists(Scenario scenario)
        {
            ...
            if (emptySolutions.Any())
            {
                ...
                if (empty == null)
                {
                    ...
                    empty = new Solution()
                    {
                        SolutionDescription = _localizationManager.GetString("Business_EmptySolution"),
                        IsEmpty = true,
                        Approved = true,
                    };
        ...
        public void UdpateSolutionsApprovedState(Scenario scenario)
        {
            // Appliquer l'état Approved
        ...
        public async Task<IDictionary<ProcessReferentialIdentifier, bool>> GetReferentialsUse(KsmedEntities context, int projectId) =>
