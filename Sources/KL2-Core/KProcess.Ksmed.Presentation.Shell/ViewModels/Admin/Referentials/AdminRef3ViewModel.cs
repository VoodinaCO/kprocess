...
{
    /// <summary>
    /// Représente le modèle de vue de l'écran de gestion des lieux.
    /// </summary>
    class AdminRef3ViewModel : ViewModelBase, IAdminRef3ViewModel
    {
        ...
        {
            var (Referentials, Processes) = await ServiceBus.Get<IReferentialsService>().GetReferentials(ProcessReferentialIdentifier.Ref3);
        ...
        {
            await ServiceBus.Get<IReferentialsService>().SaveReferentials(items.Cast<Ref3>());
            await LoadItems();
        }
