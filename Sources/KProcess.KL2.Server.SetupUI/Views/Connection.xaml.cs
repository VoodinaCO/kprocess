...
namespace KProcess.KL2.Server.SetupUI.Views
{
    ...
    public partial class Connection : InstallScreen
    {
        ...
        private void SFTP_Storage_Checked(object sender, RoutedEventArgs e)
        {
            MainViewModel.Instance.FileProvider = "SFtp";
            ((DelegateCommand)MainViewModel.Instance.TestConnectionsCommand).RaiseCanExecuteChanged();
            MainViewModel.Instance.SetConnectionTestOK(false);
        }

        private void LocalStorage_Checked(object sender, RoutedEventArgs e)
        {
            MainViewModel.Instance.FileProvider = "Local";
            ((DelegateCommand)MainViewModel.Instance.TestConnectionsCommand).RaiseCanExecuteChanged();
            MainViewModel.Instance.SetConnectionTestOK(false);
        }
    }
